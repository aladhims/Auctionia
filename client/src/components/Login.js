...
class Login extends Component {
    ...
    async handleLogin (e) {
        ...
        try {
            ...
            console.log(err)
            console.log(err.message.split(' ').splice(2).join(' '));
            this.props.hideLoading();
            this.props.showMessage(err.message.split(' ').splice(2).join(' '))
        }
            ...
            <div className="login-page">
                <div className="form">
                    <form className="login-form" ref="toggle">
                        ...
                        <p className="message">Belum Daftar? <Link to="/signup" >Buat Akun</Link></p>
                    </form>
                </div>
            </div>
        )
...
export default connect(null,acts)(graphql(LOGIN_MUTATION,{name: 'authenticate'})(Login));