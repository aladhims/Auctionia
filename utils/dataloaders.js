...
const loaders = ({User,Auction,Transaction,Bid}) =>({
    auctionByIDLoader: new DataLoader(
        keys => Promise.all(keys.map(async key => await Auction.find({_id: key}))),
        {cacheKeyFn: key => key.toString()},
    ),
    auctionByParticipantsLoader: new DataLoader(
        keys => Promise.all(keys.map(async key => await Auction.find({biddersId: key}))),
        {cacheKeyFn: key => key.toString()},
    ),
    auctionByWinnersLoader: new DataLoader(
        keys => Promise.all(keys.map(async key => await Auction.find({winnerId: key}))),
        {cacheKeyFn: key => key.toString()},
    ),
    auctionByOwnersLoader: new DataLoader(
        keys => Promise.all(keys.map(async key => await Auction.find({byId: key}))),
        {cacheKeyFn: key => key.toString()},
    ),
    userAuctionByID: new DataLoader(
        keys => Promise.all(keys.map(async key => {
    ...
    userAuctionWinner: new DataLoader(
        keys => Promise.all(keys.map(async key => await User.findOne({winnerId: key}))),
        {cacheKeyFn: key => key.toString()},
    ),
    userTransactionID: new DataLoader(
        keys => Promise.all(keys.map(async key => await Transaction.find({$or: [
            {owner: key},
            {winner: key},
        ]}))),
        {cacheKeyFn: key => key.toString()},
    ...
    bidsAuction: new DataLoader(
        keys => Promise.all(keys.map(async key => await Bid.find({auctionId: key}).sort('-amount').limit(10))),
        {cacheKeyFn: key => key.toString()},
    ),
